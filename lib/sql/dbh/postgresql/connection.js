...
import { encodeBuffer } from "#lib/sql/types";
...
export default class DbhPostgreSql extends mixins( Transactions, LargeObject, Dbh ) {
    ...
    get largeObjectReadSize () {
        return this.#pool.largeObjectReadSize;
    }
        ...
        if ( !res ) {
            if ( sth.error ) {
                ...
                if ( !res ) {
                    ...
                    if ( sth.rows !== undefined ) res.meta.rows = sth.rows;
                }
            }
        }
    ...
    _ON_PG_PARAMETER_STATUS ( data ) {
        var [ key, val ] = data.toString().split( "\0", 2 );
    ...
    _ON_PG_COMMAND_COMPLETE ( data ) {
        ...
        const rows = rowsTag.match( /\s(\d+)$/v );
    ...
    _ON_PG_NOTIFICATION_RESPONSE ( data ) {
        ...
        var [ name, payload ] = data.subarray( 4 ).toString().split( "\0", 2 );
                    ...
                    if ( param == null ) {
                        ...
                        if ( typeof param !== "string" ) {
                            ...
                            // object
                                ...
                                if ( ArrayBuffer.isView( param ) ) {
                                    if ( !Buffer.isBuffer( param ) ) {
                                        param = Buffer.from( param.buffer, param.byteoffset, param.byteLength );
                                    }
                                ...
                                // ArrayBuffer
                                    ...
                                    param = encodeBuffer( Buffer.from( param ) );
                                }
                                ...
                                // Temporal.Instant
                                else if ( param instanceof Temporal.Instant ) {
                                    param = param.toString();
                                }

                                // Temporal.ZonedDateTime
                                else if ( param instanceof Temporal.ZonedDateTime ) {
                                    param = param.toInstant().toString();
                                }
                            ...
                            // error
                                ...
                                throw new TypeError( `Unsupported SQL parameter type "${ param }"` );
                            }
                        }
