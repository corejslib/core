...
export class Ejs {
    ...
    constructor ( template, { cache, async, escape, strictGlobals, strictReferences } = {} ) {
...
`.trim(),
            ...
            if ( this.#async ) {
                this.#render = async function ( locals, context ) {
                    try {
                        return await generate( proxy, locals ).call( context );
                    }
                    catch ( e ) {
                        throw `Error rendering EJS template\n${ e }\n${ this.template.replaceAll( /^/gmv, "| " ) }`;
                    }
                };
            }
            else {
    ...
    render ( locals ) {
        const context = {
            "escape": this.#escape === undefined
                ...
                : this.#escape,
            stringify,
            "strictGlobals": this.#strictGlobals,
            "strictReferences": this.#strictReferences,
            "output": [],
        };

        return this.#render( locals, new Proxy( context, CONTEXT_PROXY ) );
    }
