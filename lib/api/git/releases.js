...
You need to merge with the "${ latestPreRelease.versionString }" first.`,
    ...
    getReleaseTagsVersions ( { majorTags = true } = {} ) {
        const tags = {
            "latest": null,
            "next": null,
        };

        for ( const release of this ) {

            // next
            if ( tags.next ) {
                if ( release.gt( tags.next ) ) {
                    tags.next = release;
                }
            }
            else {
            ...
            if ( !release.isPreRelease ) {
                ...
                // major
                if ( majorTags ) {
                    const major = "v" + release.majorNumber;

                    if ( tags[ major ] ) {
                        if ( release.gt( tags[ major ] ) ) {
                            tags[ major ] = release;
                        }
                    }
                    else {
