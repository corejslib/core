...
export default class HttpMessage {
    ...
    checkCompression ( headers, { createBody, compress, zlibOptions } = {} ) {
        ...
        if ( !mimeType?.compressible ) if ( !compress ) return;

        const acceptEncoding = headers.acceptEncoding;
        if ( !acceptEncoding ) if ( !compress ) return;

        for ( let encoding of acceptEncoding ) {
            if ( encoding === "*" ) encoding = DEFAULT_COMPRESSOR;
