...
export function checkFsCaseSensitiveSync () {
    if ( isFsCaseSensitive == null ) {
        const originalPath = process.argv[ 0 ];

        let testPath = originalPath.toUpperCase();

        if ( testPath === originalPath ) {
            testPath = originalPath.toLowerCase();
        }

        isFsCaseSensitive = !pathExistsSync( testPath );
    }
