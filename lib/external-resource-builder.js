...
export default class ExternalRecourceBuilder {
    ...
    constructor ( id, { githubtoken } = {} ) {
        ...
        const [ owner, repo, tag, name ] = this.id.split( "/", 4 );
    ...
    static async build ( resources, { force, patterns } = {} ) {
        ...
        for ( const resource of resources ) {
            process.stdout.write( `Building resource "${ resource.id }" ... ` );

            let res;

            if ( patterns && !patterns.test( resource.id, { "caseSensitive": false } ) ) {
    ...
    async #build ( { force } = {} ) {
        ...
        res = await this.#uploadAsset( new File( {
            "path": this.#name + ".json",
            "sources": JSON.stringify( index, null, 4 ) + "\n",
        } ) );
        if ( !res.ok ) return res;
    ...
    async #prepareEtag ( etag ) {
        ...
        if ( !( etag instanceof crypto.Hash ) ) {
            ...
            // Temporal.Instant
            else if ( etag instanceof Temporal.Instant ) {
                etag = etag.toString();
            }

            // Temporal.ZonedDateTime
            else if ( etag instanceof Temporal.ZonedDateTime ) {
                etag = etag.toInstant().toString();
            }
